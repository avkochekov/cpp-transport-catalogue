...
void Base_Request(const json::Node &node, catalogue::TransportCatalogue &catalogue)
{
...
void Stat_Request(const json::Node &node, catalogue::TransportCatalogue &catalogue, MapRenderer &renderer, std::ostream& stream)
{
    using namespace json;
    ...
    for (const Node &node : nodes){
        ...
        if (dict.at("type") == "Bus"){
            ...
            Render_Map_Request(catalogue, renderer);
...
void Render_Settings_Request(const json::Node &node, renderer::MapRenderer &render)
{
    ...
    render.underlayer_color = Render_Color(nodes.at("underlayer_color"));
    ...
    for(const auto &color_node : color_palette){
        render.color_palette.push_back(Render_Color(color_node));
    }
}
